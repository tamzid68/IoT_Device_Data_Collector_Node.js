...
export const ingestDataValidator = [
    ...
    body('temperature')
        ...
        .isFloat({ min: -10.0, max: 60.0 }).withMessage('Temperature must be a number.'),

    // Validate 'humidity' if it exists.
    body('humidity')
        .optional({ checkFalsy: true })
        .isFloat({ min: 0.0, max: 100.0 }).withMessage('Humidity must be a number.'),

    // Validate 'timestamp' if it exists and convert it to a Date object.
    body('timestamp')
        .optional().isISO8601()
        .withMessage('If provided, timestamp must be a valid ISO 8601 date string.').toDate(),
